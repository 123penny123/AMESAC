...
class MTMHSAC_Agent():
    def __init__(self,
        ...
        create_directory(self.agent_config.datadir)
    ...
    def _evaluate(self, steps, load=False):
        if load:
            self._load_model()
        success = 0
        for idx in range(self.num_tasks):
    ...
    def evaluate(self, load=False, render=False):
        if load:
            self._load_model()
            ...
            for i in range(self.agent_config.eval_episodes):
                ...
                while True:
                    ...
                    determinisitc = True
                        ...
                        act = self._action(obs, determinisitc, plot)
                    act = act.squeeze(0)
                    obs, reward, done, info = self.eval_envs.step(act, idx)
