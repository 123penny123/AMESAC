...
class MixExpertAttentionGaussianPolicy(nn.Module):
    ...
    def forward(self, state, deterministic=False, with_logprob=True): 
        ...
        if plot:
            return pi_action, logp_pi, expert_value, attention, embedding_query
        else:
...
class MixExpertAttentionSACNetwork(nn.Module):
    ...
    def act(self, state, deterministic=False, plot=False):        
        with torch.no_grad():
            if plot:
                a, _, expert, attention, query = self.pi(state, deterministic, False, plot)
                return a.detach().cpu().numpy(), expert.detach().cpu().numpy(), attention.detach().cpu().numpy(), query.detach().cpu().numpy()
            else:
                a, _, _ = self.pi(state, deterministic, False, plot)
